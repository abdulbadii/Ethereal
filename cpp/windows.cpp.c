...
// static int bestGroup(int index) {
    ...
    ////bestGroup() retrieves logical processor information using Windows specific API and returns the best group id for the thread with a given index. Original code from Texel by Peter Österlund. Current code from Stockfish authors.
    ...
    /////First call to get returnLength. We expect it to fail due to the NULL buffer we pass
    ...
    ////Run as many threads as possible on the same node until core limit is reached, then move on filling the next node.
    // for (int n = 0; n < nodes; n++)
        // for (int i = 0; i < cores / nodes; i++)
            // groups[groupSize++] = n;

    ////In case a core has more than one logical processor (we assume 2) and we still have threads to allocate, then spread them across available nodes.
    // for (int t = 0; t < threads - cores; t++)
        // groups[groupSize++] = t % nodes;

    ////If we still have more threads than the total number of logical processors then return -1 and let the OS to decide what to do.
